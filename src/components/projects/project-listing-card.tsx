...
function statusPillClass(status: string) {
  ...
  if (s.includes("deliver")) return "border-primary-hover/55 bg-primary-hover/25 text-white";
  if (s.includes("develop")) return "border-white/35 bg-white/15 text-white";
  if (s.includes("operat")) return "border-white/30 bg-black/12 text-white";
  return "border-white/25 bg-black/12 text-white/95";
}
    ...
    <Link href={`/projects/${project.slug}`} className="group block h-full outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background">
      <Card className="relative h-full min-h-[24rem] overflow-hidden border border-border bg-primary py-0 shadow-xl ring-0 transition-all duration-500 ease-out hover:scale-100 hover:shadow-2xl sm:min-h-[26rem]">
        <div className="relative min-h-[20rem] flex-1 overflow-hidden sm:min-h-[22rem]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-primary/36 from-38% via-primary/18 via-58% to-primary/5 to-100% transition-opacity duration-500 group-hover:from-primary/40 group-hover:via-primary/22" aria-hidden />
          <div className="absolute left-4 top-4 flex flex-wrap gap-2 sm:left-5 sm:top-5">
            <span className="rounded-full border border-white/20 bg-black/15 px-2.5 py-1 text-[0.6rem] font-semibold uppercase tracking-[0.2em] text-white/95 backdrop-blur-md">{projectTypeLabels[project.type]}</span>
          ...
          <span className="absolute right-4 top-4 flex size-11 items-center justify-center rounded-full border border-white/25 bg-black/15 text-white opacity-0 backdrop-blur-md transition-all duration-300 group-hover:opacity-100 sm:right-5 sm:top-5">
