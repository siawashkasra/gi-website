    ...
    <div className="space-y-6">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-border bg-primary shadow-2xl shadow-primary/20 ring-1 ring-black/5">
        <div className="relative aspect-[16/10] w-full min-h-[280px] sm:aspect-[2/1] sm:min-h-[320px] lg:min-h-[380px]">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-primary/28 from-40% via-transparent to-primary/10 to-100%" aria-hidden />
          <div className="absolute bottom-4 left-4 rounded-full bg-black/20 px-3 py-1 text-xs font-semibold tabular-nums text-white backdrop-blur-md">
            {index + 1} / {len}
          </div>
          <Button type="button" variant="outline" size="icon" onClick={() => go(-1)} className="absolute left-3 top-1/2 size-11 -translate-y-1/2 rounded-full border-white/30 bg-black/15 text-white backdrop-blur-md hover:bg-black/25 hover:text-white" aria-label="Previous image">
            <ChevronLeft className="size-5" />
          </Button>
          <Button type="button" variant="outline" size="icon" onClick={() => go(1)} className="absolute right-3 top-1/2 size-11 -translate-y-1/2 rounded-full border-white/30 bg-black/15 text-white backdrop-blur-md hover:bg-black/25 hover:text-white" aria-label="Next image">
            <ChevronRight className="size-5" />
          </Button>
        </div>
        <div className="flex gap-2 overflow-x-auto border-t border-white/10 bg-black/20 p-3 sm:p-4">
          {images.map((src, i) => (
