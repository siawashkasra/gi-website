...
export function MegaMenu({ open, onNavigate, projects }: { open: boolean; onNavigate: () => void; projects: Project[] }) {
  ...
  return (
    <AnimatePresence>
      {open ? (
        <motion.div key="mega" initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }} className="absolute inset-x-0 top-full border-b border-border/80 bg-background shadow-2xl shadow-[0_24px_48px_-24px_rgba(13,27,62,0.25)]" role="dialog" aria-label={t("ariaLabel")}>
                ...
                <li key={p.slug}>
                  <Link href={`/projects/${p.slug}`} onClick={onNavigate} className="group flex gap-4 py-4 first:pt-0 transition-colors hover:bg-muted/30">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-1 [font-variant-caps:small-caps] text-[0.7rem] font-medium tracking-[0.14em] text-gi-gold">{p.category}</p>
                    </div>
                  </Link>
                </li>
              ))}
            </ul>
            <div className="flex flex-col justify-between border-t border-border/60 pt-8 lg:border-s lg:border-t-0 lg:ps-10 lg:pt-0">
              <div>
                <p className="font-heading text-2xl font-semibold leading-tight tracking-tight text-gi-navy md:text-[1.65rem]">{t("tagline")}</p>
                <dl className="mt-8 grid grid-cols-2 gap-6 text-start">
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">{t("portfolio")}</dt>
                    <dd className="mt-1 font-heading text-3xl font-bold text-gi-navy">{portfolioStats.projectCount}</dd>
                    <dd className="text-xs text-muted-foreground">{t("developments")}</dd>
                  </div>
                  <div>
                    <dt className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">{t("sectors")}</dt>
                    <dd className="mt-1 font-heading text-3xl font-bold text-gi-navy">{portfolioStats.sectorCount}+</dd>
                    <dd className="text-xs text-muted-foreground">{t("focusAreas")}</dd>
                  </div>
                </dl>
              </div>
