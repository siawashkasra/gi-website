...
export function PropertyListingsSection({ projectName, listings }: PropertyListingsSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden border-t border-border/60 bg-gradient-to-b from-section to-primary/5 py-16 sm:py-20 lg:py-24" aria-labelledby="listings-heading">
      ...
      <div className="ds-container">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="flex items-center gap-2.5">
              ...
              <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.26em] text-primary/55">Listings</p>
            </div>
            <h2 id="listings-heading" className="mt-4 font-heading text-[clamp(1.65rem,3.2vw,2.35rem)] font-semibold leading-tight tracking-tight text-foreground">
        ...
        <Card className="gi-depth-panel mt-10 rounded-2xl border border-primary/12 bg-card/95">
                  ...
                  {availabilityOptions.map((opt) => (
                    <button
                      ...
                      className={cn(
                        ...
                        availability === opt ? "border-2 border-primary bg-primary text-primary-foreground" : "border border-border bg-muted/40 text-muted-foreground hover:border-primary/30 hover:bg-muted"
                      )}
                    >
        ...
        {filtered.length === 0 ? (
          ...
          <ul className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {filtered.map((unit) => (
              <li key={unit.id}>
                <article className="gi-card-elevated group h-full overflow-hidden rounded-2xl border border-primary/10 bg-card transition-all duration-500 hover:-translate-y-1 hover:border-gi-gold/40">
                  <div className="relative aspect-[4/3] overflow-hidden">
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-foreground/25 to-transparent opacity-80" aria-hidden />
                    <span className="absolute left-4 top-4 rounded-full border border-white/25 bg-black/20 px-3 py-1 text-[0.65rem] font-semibold uppercase tracking-wider text-white backdrop-blur-md">{propertyListingTypeLabels[unit.type]}</span>
                  ...
                  <div className="space-y-3 p-6">
                    ...
                    <p className="font-sans text-2xl font-semibold tabular-nums tracking-tight text-foreground">{formatPrice(unit.priceUsd)}</p>
                    <p className="text-sm text-muted-foreground">
                      <span className="font-semibold text-foreground">{unit.sizeSqm}</span> m² interior / lettable (indicative)
                    </p>
                    <Button render={<Link href={`/contact?interest=${encodeURIComponent(projectName)}&unit=${encodeURIComponent(unit.id)}`} />} nativeButton={false} variant="outline" size="sm" className="mt-2 w-full rounded-xl border-primary/20 font-semibold text-primary hover:border-primary/35 hover:bg-primary/5">
