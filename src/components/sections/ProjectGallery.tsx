...
export function ProjectGallery({ images, projectName }: { images: string[]; projectName: string }) {
  ...
  return (
    <section className="relative overflow-hidden border-b border-white/10 bg-gi-navy py-16 text-white md:py-24" aria-labelledby="project-gallery-heading">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_100%_55%_at_100%_0%,rgba(255,255,255,0.1),transparent_52%)]" aria-hidden />
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(195deg,rgba(255,255,255,0.05)_0%,transparent_40%)]" aria-hidden />
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_50%_45%_at_0%_100%,rgba(201,168,76,0.07),transparent_55%)]" aria-hidden />
          ...
          <div ref={scrollerRef} className="flex snap-x snap-mandatory gap-4 overflow-x-auto overflow-y-hidden px-4 pb-5 pt-2 scrollbar-gi sm:gap-6 md:px-8 md:pb-6" style={{ WebkitOverflowScrolling: "touch" }}>
            {images.map((src, i) => (
              <div
                ...
                className={cn(
                  "relative shrink-0 cursor-pointer snap-center overflow-hidden rounded-2xl border border-white/15 bg-gi-navy/30 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.08),0_24px_56px_-28px_rgba(5,13,31,0.55)] ring-1 ring-white/5 transition-[border-color,box-shadow] duration-500 hover:border-gi-gold/35 hover:shadow-[0_28px_64px_-24px_rgba(13,27,62,0.55)]",
