...
export function MilestonesSection() {
  ...
  return (
    <section id="milestones" className="ds-section border-b border-border/60 bg-background" aria-labelledby="milestones-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-section to-primary/5 px-7 py-9 sm:px-9 sm:py-10 lg:px-11 lg:py-11 xl:px-12">
            ...
            <h2 id="milestones-heading" className="mt-3 max-w-2xl font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-foreground">Milestones</h2>
          ...
          <div className="grid lg:grid-cols-[minmax(0,1fr)_minmax(0,0.44fr)] xl:grid-cols-[minmax(0,1fr)_minmax(0,0.4fr)]">
            <div className="relative order-2 border-t border-border/50 bg-card px-7 py-9 sm:px-9 sm:py-10 lg:order-1 lg:border-r lg:border-t-0 lg:border-border/50 lg:px-10 lg:py-11 xl:px-12 xl:py-12">
              <motion.ol className="relative m-0 list-none space-y-0 border-l border-primary/12 pl-7 sm:pl-8" variants={list} initial={reduce ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-32px" }}>
                {milestones.map((m) => (
                  <motion.li key={m.title} variants={item} className="relative pb-12 pl-1 last:pb-0">
                    <span className="absolute -left-7 top-1.5 size-3 -translate-x-1/2 rounded-full border-2 border-white bg-primary outline outline-[3px] outline-primary/20 sm:-left-8" aria-hidden />
                    <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.24em] text-primary/90">{m.year}</p>
                    <h3 className="mt-2 font-heading text-lg font-semibold tracking-tight text-foreground sm:text-xl">{m.title}</h3>
            ...
            <div className="relative order-1 min-h-[13rem] border-b border-border/50 sm:min-h-[16rem] lg:order-2 lg:min-h-0 lg:self-stretch lg:border-b-0">
              <div className="group/image relative h-full min-h-[13rem] sm:min-h-[16rem] lg:sticky lg:top-28 lg:min-h-[min(36rem,78vh)]">
                ...
                <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,rgba(13,27,62,0.325)_0%,rgba(13,27,62,0.06)_42%,transparent_68%)]" aria-hidden />
                <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(125deg,rgba(13,27,62,0.14)_0%,transparent_45%)]" aria-hidden />
