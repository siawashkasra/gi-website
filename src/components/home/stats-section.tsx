...
function StatBlock({ icon: Icon, children, label, sublabel, className }: StatBlockProps) {
  return (
    <div
      className={cn(
        "group relative overflow-hidden rounded-2xl border border-border/70 bg-card p-8 shadow-sm transition-all duration-500 hover:border-primary/25 hover:shadow-[0_20px_40px_-16px_rgba(0,0,0,0.12)] hover:shadow-primary/[0.07]",
        className
      )}
    >
      ...
      <div className="relative mt-6 font-serif text-4xl font-medium tracking-tight text-foreground sm:text-[2.75rem] sm:leading-none">{children}</div>
