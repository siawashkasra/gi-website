...
import { homeHighlights, leadFromBody } from "@/data/company-profile";
import { getLocalizedCompetitiveStrengths, getLocalizedGovernanceIntro, getLocalizedInternationalPresenceStrings, type Messages } from "@/lib/i18n/localized-data";
...
export function StrengthsInternationalHomeSection() {
  ...
  const tSnapshot = useTranslations("home.snapshot");
  const messages = useMessages() as Messages;
  const competitiveStrengths = getLocalizedCompetitiveStrengths(messages);
  const internationalPresence = getLocalizedInternationalPresenceStrings(messages);
  const { formatNumber, localizeText } = useLocalizedFormat();
  const governanceTeaser = getLocalizedGovernanceIntro(messages)[0] ?? "";
  ...
  const intlBlocks = [
    { icon: Globe2, title: t("uae"), body: leadFromBody(internationalPresence.uae, 220) },
    { icon: Sparkles, title: t("regional"), body: leadFromBody(internationalPresence.regional, 220) },
    { icon: Shield, title: t("crossBorder"), body: leadFromBody(internationalPresence.crossBorder, 220) },
  ] as const;
  return (
        ...
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 shadow-[0_28px_90px_-42px_rgba(13,27,62,0.18)]" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div id="home-governance" className="relative bg-gi-navy px-7 py-10 text-white sm:px-9 sm:py-11 lg:px-11 lg:py-12 xl:px-12">
            ...
            <div className="relative max-w-3xl">
              <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-white/55">{tSnapshot("governance")}</p>
              <p className="mt-3 font-sans text-sm leading-relaxed text-white/78 sm:text-[0.9375rem] sm:leading-[1.65]">{localizeText(governanceTeaser)}</p>
          ...
          <div className="relative border-t border-border/50 bg-gradient-to-b from-white to-gi-navy/[0.02]">
            ...
            <div className="px-7 pb-2 pt-10 text-center sm:px-9 sm:pt-11 lg:px-11 lg:pt-12 xl:px-12">
              ...
              <h2 id="positioning-heading" className="mx-auto mt-3 max-w-2xl font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-gi-navy">{t("title")}</h2>
            ...
            <motion.div className="mx-auto grid max-w-6xl grid-cols-1 gap-6 px-7 pb-10 pt-8 sm:px-9 lg:grid-cols-3 lg:gap-7 lg:px-11 lg:pb-11 xl:px-12" variants={list} initial={reduce ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-24px" }}>
              {teaserStrengths.map((s) => (
                <motion.article key={s.order} variants={item} className="group relative flex flex-col overflow-hidden rounded-2xl border border-gi-navy/[0.08] bg-white p-7 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.9)] transition-[border-color,box-shadow] duration-500 hover:border-gi-navy/18 hover:shadow-[0_20px_56px_-36px_rgba(13,27,62,0.14)]">
                  <div className="pointer-events-none absolute -end-5 -top-5 size-20 rounded-full bg-primary/[0.05] transition-transform duration-500 group-hover:scale-110" aria-hidden />
                  <div className="relative flex size-10 items-center justify-center rounded-lg bg-primary/10 font-sans text-lg font-semibold tabular-nums text-primary ring-1 ring-primary/12">{formatNumber(s.order)}</div>
                  <h3 className="relative mt-4 font-heading text-lg font-semibold tracking-tight text-gi-navy">{s.title}</h3>
                  <p className="relative mt-2 font-sans text-sm leading-relaxed text-muted-foreground">{localizeText(leadFromBody(s.body, 200))}</p>
                </motion.article>
              ))}
            </motion.div>
              ...
              {intlBlocks.map((block) => {
                ...
                return (
                  <motion.article key={block.title} variants={item} className="group relative flex flex-col overflow-hidden rounded-2xl border border-gi-navy/[0.08] bg-white p-6 shadow-[inset_0_1px_0_0_rgba(255,255,255,0.9)] transition-[border-color,box-shadow] duration-500 hover:border-gi-navy/18 hover:shadow-[0_20px_56px_-36px_rgba(13,27,62,0.14)] sm:p-7">
                    ...
                    <p className="relative mt-2 font-sans text-sm leading-relaxed text-muted-foreground">{localizeText(block.body)}</p>
                  </motion.article>
                );
              })}
            </motion.div>
            ...
            <p className="mx-auto max-w-3xl border-t border-border/50 px-7 py-6 text-center font-sans text-[0.6875rem] leading-relaxed text-muted-foreground sm:px-9 sm:text-xs lg:px-11 xl:px-12">{t("footnote")}</p>
          </div>
        </motion.div>
      </div>
    </section>
