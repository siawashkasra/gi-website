...
export function AboutSection() {
  ...
  return (
    <section id="about" className="ds-section border-b border-border/60 bg-section" aria-labelledby="about-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel lg:grid lg:min-h-[min(28rem,72vh)] lg:grid-cols-[minmax(0,0.46fr)_minmax(0,1fr)]" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.78, ease: easeLuxury }}>
          <div className="group/image relative min-h-[14rem] border-b border-border/50 sm:min-h-[18rem] lg:min-h-full lg:border-b-0 lg:border-r lg:border-border/50">
            ...
            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,rgba(13,27,62,0.36)_0%,rgba(13,27,62,0.09)_38%,transparent_62%)]" aria-hidden />
            <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(115deg,rgba(13,27,62,0.175)_0%,transparent_48%)]" aria-hidden />
          ...
          <div className="relative bg-gradient-to-b from-section to-primary/5 p-7 sm:p-8 lg:flex lg:flex-col lg:justify-center lg:p-10 xl:p-12">
            ...
            <div>
              ...
              <h2 id="about-heading" className="mt-3 font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-foreground">{companyAbout.headline}</h2>
            ...
            <div className="mt-10">
              <Button render={<Link href={homeHighlights.companyCtaPath} />} nativeButton={false} variant="outline" size="lg" className="h-12 rounded-xl border-primary/20 bg-card px-8 font-semibold text-primary shadow-sm transition-all hover:scale-100 hover:border-primary/35 hover:bg-primary/5 hover:text-primary">
