...
export function CeoMessageSection() {
  ...
  return (
    <section id="ceo-message" className="ds-section border-b border-border/60 bg-section" aria-labelledby="ceo-message-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel lg:grid lg:min-h-[min(26rem,70vh)] lg:grid-cols-[minmax(0,0.42fr)_minmax(0,1fr)]" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.78, ease: easeLuxury }}>
          <div className="relative order-2 min-h-[15rem] border-b border-border/50 sm:min-h-[18rem] lg:order-1 lg:min-h-0 lg:border-b-0 lg:border-r lg:border-border/50">
            <div className="group/image relative h-full min-h-[15rem] sm:min-h-[18rem] lg:min-h-[min(32rem,72vh)]">
              ...
              <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_top,rgba(13,27,62,0.34)_0%,rgba(13,27,62,0.075)_40%,transparent_65%)]" aria-hidden />
              <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(118deg,rgba(13,27,62,0.16)_0%,transparent_48%)]" aria-hidden />
          ...
          <div className="relative order-1 bg-gradient-to-b from-section to-primary/5 px-7 py-9 sm:px-9 sm:py-10 lg:order-2 lg:flex lg:flex-col lg:justify-center lg:px-10 lg:py-11 xl:px-12 xl:py-12">
            ...
            <motion.div variants={list} initial={reduce ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-28px" }}>
              <motion.div variants={item}>
                ...
                <h2 id="ceo-message-heading" className="mt-3 font-heading text-[clamp(1.65rem,3vw,2.35rem)] font-semibold leading-[1.08] tracking-tight text-foreground">Message From The CEO</h2>
              ...
              <motion.div variants={item} className="mt-8">
                <p className="max-w-2xl font-heading text-[1.0625rem] font-normal leading-[1.62] text-foreground sm:text-lg sm:leading-[1.65]">&ldquo;{ceoProfile.quote}&rdquo;</p>
              </motion.div>
              <motion.footer variants={item} className="mt-10 border-t border-border/55 pt-8">
                <p className="font-heading text-2xl font-semibold tracking-tight text-foreground sm:text-[1.75rem]">{ceoProfile.name}</p>
