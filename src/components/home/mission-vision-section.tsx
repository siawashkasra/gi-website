...
export function MissionVisionSection() {
  ...
  return (
    <section id="mission-vision" className="ds-section border-b border-border/60 bg-section" aria-labelledby="mission-vision-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-section to-primary/5 px-7 py-9 text-center sm:px-9 sm:py-10 lg:px-11 lg:py-11 xl:px-12">
            ...
            <h2 id="mission-vision-heading" className="mx-auto mt-3 max-w-2xl font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-foreground">Mission & vision</h2>
          ...
          <motion.div className="grid md:grid-cols-2" variants={list} initial={reduce ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-24px" }}>
            <motion.div variants={item} className="relative border-b border-border/50 bg-card px-7 py-9 sm:px-9 sm:py-10 md:border-b-0 md:border-r md:border-border/50 lg:px-11 lg:py-12 xl:px-12 xl:py-14">
              <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_90%_70%_at_0%_0%,rgba(13,27,62,0.02),transparent_55%)]" aria-hidden />
              <div className="relative">
                <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.22em] text-primary/90">Purpose</p>
                <h3 className="mt-3 font-heading text-2xl font-semibold tracking-tight text-foreground sm:text-[1.75rem]">Our Mission</h3>
            ...
            <motion.div variants={item} className="relative bg-primary px-7 py-9 text-white sm:px-9 sm:py-10 lg:px-11 lg:py-12 xl:px-12 xl:py-14">
          ...
          <p className="border-t border-border/50 bg-card px-7 py-6 text-center font-sans text-[0.6875rem] leading-relaxed text-muted-foreground sm:px-9 sm:text-xs lg:px-11">Board-level positioning statements; operating detail and sector programs are described throughout the company profile and portfolio.</p>
        </motion.div>
      </div>
    </section>
  );
