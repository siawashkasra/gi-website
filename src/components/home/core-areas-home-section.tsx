...
export function CoreAreasHomeSection() {
  ...
  return (
    <section id="core-areas" className="ds-section border-b border-border/60 bg-background" aria-labelledby="core-areas-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-section to-primary/5 px-7 py-9 text-center sm:px-9 sm:py-10 lg:px-11 lg:py-11 xl:px-12">
            ...
            <h2 id="core-areas-heading" className="mx-auto mt-3 max-w-2xl font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-foreground">Core business areas</h2>
          ...
          <div className="relative bg-card px-7 py-9 sm:px-9 sm:py-10 lg:px-10 lg:py-11 xl:px-12 xl:py-12">
            <motion.div className="mx-auto grid max-w-5xl gap-6 lg:grid-cols-2 lg:gap-8" variants={list} initial={reduce ? "visible" : "hidden"} whileInView="visible" viewport={{ once: true, margin: "-28px" }}>
              {coreBusinessAreas.map((area, i) => {
                ...
                return (
                  <motion.article key={area.title} variants={item} className="group relative flex flex-col overflow-hidden rounded-2xl border border-primary/10 bg-card p-8 gi-card-elevated transition-[border-color,filter] duration-500 hover:border-primary/18 lg:p-10">
                    <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_85%_55%_at_100%_0%,rgba(13,27,62,0.022),transparent)]" aria-hidden />
                    ...
                    <h3 className="relative mt-6 font-heading text-xl font-semibold tracking-tight text-foreground sm:text-2xl">{area.title}</h3>
                    ...
                    <div className="relative mt-7 flex flex-wrap gap-3">
                      ...
                      <Button render={<Link href={`${homeHighlights.companyCtaPath}#core-areas`} />} nativeButton={false} variant="outline" size="sm" className="rounded-xl border-primary/18 px-4 font-semibold text-foreground hover:scale-100 hover:border-primary/30 hover:bg-primary/[0.04] hover:text-foreground">
                        Read full context
                      </Button>
                    </div>
                  </motion.article>
