...
export function StandardsSection() {
  ...
  return (
    <section id="standards" className="ds-section border-b border-border/60 bg-section" aria-labelledby="standards-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 gi-depth-panel" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-section to-primary/5 px-7 py-9 text-center sm:px-9 sm:py-10 lg:px-11 lg:py-11 xl:px-12">
            ...
            <h2 id="standards-heading" className="mx-auto mt-3 max-w-2xl font-heading text-[clamp(1.65rem,3.2vw,2.65rem)] font-semibold leading-[1.08] tracking-tight text-foreground">How we work</h2>
              ...
              {standardPillars.map((p, i) => {
                ...
                return (
                  <motion.li key={p.title} variants={item}>
                    <article className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-primary/10 bg-card p-7 text-center gi-card-elevated transition-[border-color,filter] duration-500 hover:border-primary/18 sm:p-8 lg:text-left">
                      ...
                      <h3 className="relative mt-5 font-heading text-xl font-semibold tracking-tight text-foreground">{p.title}</h3>
