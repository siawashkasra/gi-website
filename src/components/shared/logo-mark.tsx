    ...
    <Link href="/" className={cn("group flex items-center gap-3", className)}>
      <span
        className={cn(
          ...
          variant === "dark" ? "border-foreground/15 bg-foreground text-background" : "border-white/25 bg-white/10 text-white"
        )}
        aria-hidden
      >
        GI
      ...
      <span className={cn("flex flex-col leading-tight", variant === "dark" ? "text-foreground" : "text-white")}>
        ...
        <span
          className={cn(
            ...
            variant === "dark" ? "text-muted-foreground group-hover:text-foreground/80" : "text-white/65"
          )}
        >
          Investment
        </span>
