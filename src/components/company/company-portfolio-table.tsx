...
import { getLocalizedPortfolioTableRows, type Messages } from "@/lib/i18n/localized-data";

export function CompanyPortfolioTable() {
  const t = useTranslations("company.portfolio");
  const messages = useMessages() as Messages;
  const portfolioTableRows = getLocalizedPortfolioTableRows(messages);
  return (
      ...
      <div className="ds-container">
        <div className="overflow-hidden rounded-3xl border border-border/60 shadow-[0_28px_90px_-42px_rgba(13,27,62,0.18)]">
          <div className="relative border-b border-border/50 bg-gradient-to-b from-white to-gi-navy/[0.02] px-7 py-9 sm:px-9 sm:py-10">
            ...
            <SectionHeading id="portfolio-heading" eyebrow={t("eyebrow")} title={t("title")} />
          ...
          <div className="overflow-x-auto border-t border-border/50 bg-white p-0">
            <table className="w-full min-w-[32rem] text-start text-sm">
              <thead className="bg-gradient-to-b from-gi-navy/[0.06] to-gi-navy/[0.02] text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                <tr>
                  <th className="px-5 py-4 font-sans sm:px-6">{t("project")}</th>
                  <th className="px-5 py-4 font-sans sm:px-6">{t("location")}</th>
                  <th className="px-5 py-4 font-sans sm:px-6">{t("sector")}</th>
                </tr>
              </thead>
              <tbody>
          ...
          <p className="border-t border-border/50 bg-white px-7 py-5 font-sans text-[0.6875rem] leading-relaxed text-muted-foreground sm:px-9 sm:py-6 sm:text-xs">{t("footnote")}</p>
        </div>
      </div>
    </section>
  );
