...
export function CompanySnapshotFull() {
  ...
  const tCo = useTranslations("company.snapshot");
  const tAbout = useTranslations("company.about");
  const tOrg = useTranslations("company.org");
  const messages = useMessages() as Messages;
  const rows = getLocalizedCompanySnapshot(messages);
        ...
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 shadow-[0_28px_90px_-42px_rgba(13,27,62,0.18)] lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,17.5rem)] xl:grid-cols-[minmax(0,1fr)_minmax(0,19.5rem)]" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-white to-gi-navy/[0.02] p-7 sm:p-8 lg:border-b-0 lg:border-e lg:border-border/50 lg:p-9 xl:p-11">
            ...
            <div className="relative max-w-3xl">
              ...
              <h2 id="snapshot-full-heading" className="mt-3 font-heading text-[clamp(1.65rem,3.4vw,2.5rem)] font-semibold leading-[1.08] tracking-tight text-gi-navy">{t("title")}</h2>
            ...
            <p className="mt-8 border-t border-border/50 pt-6 font-sans text-[0.6875rem] leading-relaxed text-muted-foreground sm:text-xs">{tAbout("footnote")}</p>
          </div>
          <div className="relative flex flex-col justify-between bg-gi-navy px-8 py-10 text-white sm:px-9 sm:py-11 lg:px-8 lg:py-12 xl:px-10 xl:py-14">
            ...
            <div className="relative">
              <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.26em] text-white/55">{tCo("next")}</p>
              <p className="mt-3 font-heading text-lg font-semibold leading-snug tracking-tight text-white xl:text-xl">{tCo("nextTitle")}</p>
              <p className="mt-3 font-sans text-xs leading-relaxed text-white/68 sm:text-sm">{tOrg("description")}</p>
            </div>
            <div className="relative mt-10 lg:mt-12">
              <div className="flex items-baseline gap-2 border-b border-white/10 pb-8">
                ...
                <span className="pb-1 font-sans text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-white/50">{tCo("profileFields")}</span>
              </div>
