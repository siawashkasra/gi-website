...
import { getLocalizedTechnologyStandards, type Messages } from "@/lib/i18n/localized-data";

const easeLuxury = [0.16, 1, 0.3, 1] as const;

export function CompanyTechSustainability() {
  const t = useTranslations("company.tech");
  const messages = useMessages() as Messages;
  const { technologyStandards, sustainabilityStandards } = getLocalizedTechnologyStandards(messages);
    ...
    <section id="standards-full" className="ds-section border-b border-border/60 bg-white" aria-labelledby="tech-sus-heading">
      <div className="ds-container">
        <motion.div className="overflow-hidden rounded-3xl border border-border/60 shadow-[0_28px_90px_-42px_rgba(13,27,62,0.18)] lg:grid lg:grid-cols-2" initial={reduce ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-72px" }} transition={reduce ? { duration: 0 } : { duration: 0.75, ease: easeLuxury }}>
          <div className="relative border-b border-border/50 bg-gradient-to-b from-white to-gi-navy/[0.02] px-7 py-9 sm:px-9 sm:py-10 lg:border-b-0 lg:border-e lg:border-border/50">
            ...
            <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-primary/85">{t("built")}</p>
            <h2 id="tech-sus-heading" className="mt-3 font-heading text-[clamp(1.5rem,2.5vw,2rem)] font-semibold leading-tight tracking-tight text-gi-navy">{t("techTitle")}</h2>
          ...
          <div className="relative border-t border-border/50 bg-white px-7 py-9 sm:px-9 sm:py-10 lg:border-t-0">
            ...
            <p className="font-sans text-[0.65rem] font-semibold uppercase tracking-[0.28em] text-primary/85">{t("responsibility")}</p>
            <h3 className="mt-3 font-heading text-[clamp(1.5rem,2.5vw,2rem)] font-semibold leading-tight tracking-tight text-gi-navy">{t("sustainTitle")}</h3>
            <div className="mt-4 h-px w-16 bg-gradient-to-r from-primary/35 to-transparent" aria-hidden />
        ...
        <p className="mt-6 text-center font-sans text-[0.6875rem] leading-relaxed text-muted-foreground sm:text-xs">{t("footnote")}</p>
      </div>
    </section>
  );
}
